	...
	public void doActionToTargets() {
		...
		while (!targetStack.isEmpty()) {
			...
			if (save < action.getDc()) {
				System.out.println(action.getName() + " hits " + currentTarget + "(rolled " + save + ") with full damage: " + damageRoll);
				currentTarget.reduceHealthPoints(damageRoll);
			} else {
				int halfDamage = Math.round(damageRoll)/2;
				System.out.println(action.getName() + " hits " + currentTarget + "(rolled " + save + ") with half damage: " + halfDamage);
